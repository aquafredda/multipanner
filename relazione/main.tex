...
\section{Modello Matematico}
    ...
    % mo, caro gabriele, il modello matematico può essere così complesso da non esistere nel mondo fisico. non mi azzarderei a dire che matematico significa più semplice di fisico. Il modello è matematico per il solo fatto che non vuole, per ora, contemplare implementazioni fisiche. 
     ...
     % no, quello è il modello fisico. il modello matematico è astratto, non ha legami con il mondo reale, altrimenti sarebbe un modello fisico. 
     
     Nel modello matematico, essendoci anche una semplificazione, si tengono in considerazione solo parte dei fenomeni che avvengono realmente. % no
    ...
    % bo, mi sembra un linguaggio un po troppo da canale5… dobbiamo permettere al nostro programma di faust… è una calcolatrice, non gli stai permettendo nulla. fa solo calcoli. 
    ...
    % gabriele… con tutto il sacramento che smuove scagliola nelle sue lezioni, veramente gli vuoi scrivere la formula del teorema di pitagora? 
    ...
    % elimina tutti i nostra noi, io tu… mammeta… se vuoi parlare della libreria prima la presenti e poi la usi, ma non puoi dire la nostra libreria. 
...
\subsection{Il ritardo tra i due microfoni main}
    La velocità del suono influisce sulle differenze di tempo, quindi è necessario costruire una funzione che, prese in input le distanze tra la sorgente e ciascuna capsula microfonica, calcoli un ritardo attraverso un valore di velocità arbitrario, che in questo caso sarà $321 m/s$ % devi dire che è uno degli standard che si una negli interni auditorium
...
\section{Perché utilizzare il ``multipanner''}
    ...
    % no, semplicemente non fornisce alcuna informazione stereofonica.
...
\subsection{La velocità del suono}
    ...
    % tipo? se proponi una cosa del genere quanto meno devi fare degli esempi. altrimenti vale la regola… lascia perde
